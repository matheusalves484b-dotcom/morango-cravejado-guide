...
function SalesPage() {
  ...
  return <main className="sales-page">
    ...
    <section className="section testimonials-section"><div className="container center narrow"><div className="eyebrow dark">QUEM JÁ CONHECEU O GUIA</div><h2>Mais segurança para tirar a ideia do <span>papel.</span></h2><p>Relatos de quem já começou a explorar o Morango Cravejado com mais organização.</p></div><div className="testimonial-viewport"><div className="testimonial-track">{[...testimonials,...testimonials].map((testimonial,index)=>{const [quote,name,detail]=testimonial;return <article className="testimonial-card" key={`${name}-${index}`}><div><div className="testimonial-stars">★★★★★</div><p>{quote}</p></div><div className="testimonial-meta"><b>{name.charAt(0)}</b><span><strong>{name}</strong><br/>{detail}</span></div></article>})}</div></div><div className="container testimonial-note">← deslize para conhecer mais relatos →</div></section>
